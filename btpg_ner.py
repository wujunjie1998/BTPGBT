...
for i, text in tqdm(enumerate(en_text)):
    ...
    for k, align in enumerate(final_align_all[i]):
        align_candidates = []
        # for word replacement, the source word is limited to 'noun, adj, adv, verb' when targeting the NER capability
        if len(align[2][0]) == 1:
            if text_tags[align[2][0][0]][1] not in ['NOUN', 'VERB', 'ADJ', 'ADV']:
                continue
...
## Load Named Entity Data
entity = []
with open('data/entity') as f:
    for line in f:
        entity.append(literal_eval(line[:-1]))
...
for i, sent in enumerate(en_text):
    ...
    for ent in alignment:
        for ner in ne:
            if ent[0] == ner[0]:
                overlap.append(ent)
                ne_information.append(ner)
    max_count = len(overlap)
    overlap_index = [k for k in range(len(overlap))]
    ...
    for _ in range(100):
        ...
        while len(phrase_replace_list) < sample_num:
            ...
            pick_index = random.choice(overlap_index)
            phrase_candidate = overlap[pick_index]
            entity_candidate = ne_information[pick_index]
            count += 1
            if len(phrase_replace_list) == 0:
                phrase_replace_list.append(phrase_candidate)
                entity_information_list.append(entity_candidate)
            ...
            if phrase_candidate in phrase_replace_list:
                ...
                ## find inclusive relation, if the found relation overlaps with previous ones, delete it
                repeat_flag = 0
        ...
        entity_information.append(entity_information_list)
...
entity_information = entity_information[split_num-1000:split_num]

instruction = "You are given an English sentence and its Chinese translation. In each sentence, a {} has been masked with the '<mask>' token. Your task is to first fill in the masked token in the English sentence using a {} other than \'{}\' without modifying any of the unmasked tokens. Then, use the filled English sentence to fill in the masked tokens in its corresponding Chinese translation. If necessary, make modifications to the filled Chinese translation to ensure fluency while preserving the meaning. Finally, please output the filled English sentence and its filled Chinese translation in the format of \'Filled English:{}\nFilled Chinese:{}\'.\n"
...
for i, text in enumerate(train_en):
    ...
    entity = entity_information[i][0]
    prompt = copy.deepcopy(instruction).format(entity_map[entity[1]], entity_map[entity[1]], entity[0],'{}','{}')\
