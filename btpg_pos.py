...
parser = argparse.ArgumentParser(description="BTPG")
parser.add_argument(
    "--capability",
    action="store",
    type=str,
...
if args.capability == 'Others':
    ...
    for i, text in tqdm(enumerate(en_text)):
        ...
        for k, align in enumerate(final_align_all[i]):
            if len(align[2][0]) == 1:
                if text_tags[align[2][0][0]][1] in ['NOUN', 'VERB', 'ADJ', 'ADV', 'ADP']:
                    continue
                align.append(text_tags[align[2][0][0]][1])
                replace_alignment.append(align)
            else:
                word_flag = 0
                for index, _ in enumerate(align[2][0]):
                    if text_tags[align[2][0][index]][1] in ['NOUN', 'VERB', 'ADJ', 'ADV', 'ADP']:
                        word_flag = 1
                        break
                if word_flag == 1:
                    continue
    ...
    for i, text in tqdm(enumerate(en_text)):
        ...
        for k, align in enumerate(final_align_all[i]):
            ...
            if len(align[2][0]) == 1:
                if text_tags[align[2][0][0]][1] not in pos_list:
                    continue
                replace_alignment.append(align)
            else:
                word_flag = 0
                for index, _ in enumerate(align[2][0]):
                    if text_tags[align[2][0][index]][1] in pos_list:
                        word_flag = 1
                        break
                if word_flag == 0:
                    continue
...
for i, sent in enumerate(en_text):
    ...
    for _ in range(100):
        ...
        ## avoid the loop from not stopping
        count = 0
        while len(phrase_replace_list) < sample_num:
            ...
            if phrase_candidate in phrase_replace_list:
                ...
                ## find inclusions and delete them
                repeat_flag = 0
...
capability_mapping = {'NOUN': 'noun', 'VERB': 'verb', 'ADJ': 'adjective', 'ADV':'adverb', 'ADP':'adposition', 'Others':'others'}

data_path = 'data/' + capability_mapping[args.capability] + '/temp'
...
instruction = "You are given an English sentence and its Chinese translation. In each sentence, an {} has been masked with the '<mask>' token. Your task is to first fill in the masked token in the English sentence using an {} other than '{}' without modifying any of the unmasked tokens. Then, use the filled English sentence to fill in the masked token in its corresponding Chinese translation. If necessary, make modifications to the filled Chinese translation to ensure fluency while preserving the meaning. Finally, please output the filled English sentence and its filled Chinese translation in the format of \'Filled English:{}\nFilled Chinese:{}\'.\n"
...
if args.capability == 'Others':
    ...
    for i, text in enumerate(train_en):
        ...
        if train_align[i][0][3] == 'NUM':
            if len(train_align[i][0][2][0]) == 1:
                edit_word = 'number'
            else:
                edit_word = 'number phrase'

        if train_align[i][0][3] == 'PRON':
            if len(train_align[i][0][2][0]) == 1:
                edit_word = 'pronoun'
            else:
                edit_word = 'pronoun phrase'

        if train_align[i][0][3] == 'CONJ':
            if len(train_align[i][0][2][0]) == 1:
                edit_word = 'conjunction'
            else:
                edit_word = 'conjunction phrase'

        if train_align[i][0][3] == 'DET':
            if len(train_align[i][0][2][0]) == 1:
                edit_word = 'determiner'
            else:
                edit_word = 'determiner phrase'

        if train_align[i][0][3] == 'PRT':
            if len(train_align[i][0][2][0]) == 1:
                edit_word = 'particle'
            else:
                edit_word = 'particle phrase'

        if train_align[i][0][3] == 'PUNC':
            if len(train_align[i][0][2][0]) == 1:
                edit_word = 'punctuation'
            else:
                edit_word = 'punctuation phrase'
        prompt = copy.deepcopy(instruction).format(edit_word, edit_word, train_align[i][0][0], '{}', '{}', '{}') \
    ...
    for i, text in enumerate(train_en):
        ...
        zh = train_zh[i].replace(' ', '')
        if len(train_align[i][0][2][0]) == 1:
            edit_word = capability_mapping[args.capability]
        else:
            edit_word = capability_mapping[args.capability] + ' phrase'
        prompt = copy.deepcopy(instruction).format(edit_word, edit_word, train_align[i][0][0], '{}', '{}', '{}') \
                 + 'English Sentence: ' + en + '\n' + 'Chinese Translation: ' + zh
        prompt_list.append(prompt)
